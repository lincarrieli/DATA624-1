...
In this report we explore a range of linear models, tree models, and neural networks to identify a procedure that is highly accurate in predicting the pH of a previously unseen beverage. For each model, we take the following steps:  
...
We built six models in total and used MAPE and RMSE scores to evaluate model performance. The summary table below shows the models and their corresponding MAPE and RMSE scores. We noticed that distance and regression tree model performed the best, followed by linear models, and Neural Networks (nonlinear) model had the worst performance overall.
...
Our best two performing models are K-Nearest Neighbors and Random Forests, both with a MAPE of < 1.0. We chose to highligh the Random Forest model because...
