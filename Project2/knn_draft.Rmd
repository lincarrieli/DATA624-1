...
The caret package knn regression function found a minimum MAE (not MAPE) of 0.09215998 with a k value of 6. When I fit the model to the test data, I got a MAPE of 7.58%.
...
fnn_func <- function(train_x, train_y, test_x, test_y){
  ...
  for(x in c("kd_tree","cover_tree","brute")){
    for(i in 1:20){
      ...
      mape <- mean(abs(test_y - knn_fnn$pred))
...
fnn_test <- fnn_test %>%
  mutate(pe = abs(actual - fnn)/actual)

MAPE <- mean(sum(fnn_test$pe))
MAPE
...
The FNN knn regression gave a minimum MAPE of 0.1003531 with a k value of 6. Running the model on the test data, I got a MAPE of 7.58% - the same as the caret package. Unfortunately, it seems there are no differences in any of the methods...
...
kknn_func <- function(train_x, train_y, test_x, test_y){
  ...
  for(x in types){
    for(i in 1:4){
      ...
      mape <- mean(abs(test_y - ph_kknn$fitted.values))
      mape_df <- rbind(mape_df,c(i,mape,x))
    }
  }
...
kknn_test <- kknn_test %>%
  mutate(pe = abs(actual - kknn)/actual)

MAPE <- mean(sum(kknn_test$pe))
MAPE
...
This model found a minimum MAPE of 0.08286259 for a biweight kernel at a distance of 1 (Manhattan distance). When I ran the model on the test data, I got a MAPE of 6.25%. This is definitely the best MAPE I've achieved so far, so I think I'm gonna stick with this model.
