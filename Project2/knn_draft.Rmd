...
The caret package knn regression function found a minimum RMSE of 0.1361693 with a k value of 6.

Let's see if I can do better with a different package and more control. Trying to model again with the knn.reg function from the FNN package. This package seems interesting because it allows you to change the distance algorithm.
...
fnn_low_knn <- function(train_x, train_y){
  rmse_df <- data.frame(matrix(nrow = 0, ncol = 3))
  ...
  for(x in c("kd_tree","cover_tree","brute")){
    for(i in 1:20){
      ...
      rmse <- (mean(knn_fnn$residuals^2))^(1/2)
      rmse_df <- rbind(rmse_df,c(i,rmse,x))
    }
  }
  colnames(rmse_df) <- c("k", "RMSE","Type")
  rmse_df[,1] <- as.integer(rmse_df[,1])
  rmse_df[,2] <- as.numeric(rmse_df[,2])
  return(rmse_df)
}

fnn_knn_rmse <- fnn_low_knn(train[,-1], train[,1])

ggplot(data = fnn_knn_rmse, aes(x = k, y = RMSE, color = Type)) +
  geom_line() +
  geom_point()
```

The FNN knn regression gave a minimum RMSE of 0.1240583 with a k value of 6. Unfortunately, it seems there are no differences in any of the methods...
...
kknn_low <- function(train_x, train_y, test_x, test_y){
  rmse_df <- data.frame(matrix(nrow = 0, ncol = 3))
  ...
  for(x in types){
    for(i in 1:4){
      ...
      rmse <- (mean((test_y - ph_kknn$fitted.values)^2))^(1/2)
      rmse_df <- rbind(rmse_df,c(i,rmse,x))
    }
  }
  colnames(rmse_df) <- c("distance", "RMSE","Type")
  rmse_df[,1] <- as.integer(rmse_df[,1])
  rmse_df[,2] <- as.numeric(rmse_df[,2])
  return(rmse_df)
}

kknn_rmse <- kknn_low(train[,-1], train[,1], test[,-1], test[,1])

ggplot(data = kknn_rmse, aes(x = distance, y = RMSE, color = Type)) +
  geom_line() +
  geom_point()
```

This model found a minimum RMSE of 0.1185263 for a triangular kernel at a distance of 1 (Manhattan distance). This is definitely the best RMSE I've acheived so far, so I think I'm gonna stick with this model.
...
predict_features[,1] <- mapvalues(predict_features[,1], from = c("A","B","C","D",""), to = c(1,2,3,4,NA))
